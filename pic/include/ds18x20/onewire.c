...
#include <interrupt.h> // ATOMIC_BLOCK SOMETHING
#include <sdelay/sdelay.h> // delay_us
#include "onewire.h"
...
uint8_t OW_PIN_MASK; 
__near volatile uint8_t *OW_IN;
__near volatile uint8_t *OW_OUT;
__near volatile uint8_t *OW_DDR;
...
uint8_t ow_rom_search( uint8_t diff, __near uint8_t *id )
{
...
void ow_command_intern( uint8_t command, __near uint8_t *id, uint8_t with_parasite_enable )
{
	uint8_t i;
