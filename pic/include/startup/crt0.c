/*-------------------------------------------------------------------------
   crt0izs.c - SDCC pic16 port runtime start code with initialisation and
              clear RAM
...
void _stack_overflow (void) __naked;
void _stack_underflow (void) __naked;
...
void _startup (void) __naked
{
	/* check if we are runned cause of stack pointer overflow/underflow */
/*__asm
	; check if we are called cause stack is full
	btfsc STKPTR, 7
	goto  _stack_overflow
	
	; check if we are called cause stack is empty
	btfsc STKPTR, 6
	goto _stack_underflow 
