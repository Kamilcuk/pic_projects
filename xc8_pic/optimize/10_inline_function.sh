...
substitute_call_to_function_with_function_body() {
	num=0
	while IFS='\n' read -r line; do
		if echo "$line" | egrep -q '^[[:space:]]+call[[:space:]]+'"$fun"'[[:space:]]'; then
			echo "## ## ## Found call $fun!" >&2
			echo "; call '$fun' ; inlined function by $(basename $0)"
			echo "${funcbody}" | sed 's/_LABEL_/_'"$num"'_/' # take care of global unique labels
			num=$(( num + 1 ))
		else
			echo "$line"
		fi
...
remove_function_body() {
	...
	while IFS='\n' read -r line; do
		if ! $funcbody; then
			if echo "$line" | egrep -q "^;; \*\*\*\*\*\*\*\*\*\*\*\*\*\*\* function $fun \*\*\*\*\*\*\*\*\*\*\*\*\*\*\*"; then
				echo "## ## ## Found $fun body" >&2
				funcbody=true;
			fi 
			echo "$line"
		else
			if echo "$line" | egrep -q "^[[:space:]]+__end_of${fun}"; then
				funcbody=false;
				echo "## ## ## End of $fun body" >&2
			fi
		fi
	done
}
